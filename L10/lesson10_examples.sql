...
-- Другой вариант построения запроса
...
-- Вопрос
-- Можете, пожалуйста, на уроке подробнее объяснить синтаксис и что такое курсоры 
-- и где применяются процедуры/транзакции? 

-- Создадим процедуру с курсором

DROP PROCEDURE IF EXISTS posts_analysis;
DELIMITER //
CREATE PROCEDURE posts_analysis()
BEGIN
  DECLARE done BOOLEAN DEFAULT FALSE;
  DECLARE post_id INT;
  DECLARE post_body TEXT;
  
  DECLARE reading_post CURSOR FOR SELECT id, body FROM posts;
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

  OPEN reading_post;

  read_loop: LOOP
    FETCH reading_post INTO post_id, post_body;
    ...
    SELECT CONCAT(
      "Post id ", post_id, 
      IF(post_body LIKE '%forbidden%'," is wrong!", " is fine!")
      ) AS Result;
